<script>
	...
	const nextAyah = () => {
		...
		if (counter < data.surah.numAyahs) {
			...
			window.location.href = `/tahfizh/${parseInt(data.params.surah)+1}`
		}
...
<div class="pt-24 pb-28 min-h-screen flex flex-col gap-4 p-4">
	{#each Object.entries(data.ayah) as [index, ayah]}
		<a id={`ayah-${index}`} href={`/tahfizh/${data.params.surah}/${index}`} class="relative bg-white px-4 py-2 flex flex-col gap-0">

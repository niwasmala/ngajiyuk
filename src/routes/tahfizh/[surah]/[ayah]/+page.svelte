<script>
	...
	const nextAyah = () => {
		if (parseInt(data.params.ayah) < data.surah.numAyahs) {
			window.setTimeout(() => {
				window.location.href = `/tahfizh/${data.params.surah}/${parseInt(data.params.ayah)+1}?mode=${mode}`
			}, mode === 'test' ? 1500 : 0)
		}
	}
...
<div class="max-w-lg mx-auto fixed z-30 top-0 left-0 right-0 p-2 flex flex-row justify-between items-center text-neutral-800 bg-white border-b-4 border-green-500">
	<div class="flex flex-row items-center gap-4">
		<a href={`/tahfizh/${data.params.surah}`} class="py-2">
		...
		<div class="text-xl font-bold">
			...
			<select bind:value={ayah} on:change={() => window.location.href = `/tahfizh/${data.params.surah}/${ayah}`} class="bg-white ml-2 py-2">
				<option></option>
